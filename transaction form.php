<?php
    ...
    $con = mysqli_connect("localhost", "root", "", "banking system"); //database connection establish
	if(!$con)
		{
...
<div class="container">
	<form action="transaction success.php" method="post">
        <h3 >MONEY TRANSACTION</h3>
		      ...
		      <b><label for="text">From(Sender):</label></b>
		      <input type="text" id="cname" name="sender" value='<?php echo "$user";?>'><br><br>
		
		      <b><label for="text">Transfer to(Reciever):</label></b>
			  <select  name="reciever" id="dropdown" required>
						...
						<?php
							...
							$res = mysqli_query($db, "SELECT * FROM `view customers` WHERE Name!='$user'");
			  ...
			  <b><label for="email">Email-id:</label></b>
						...
						<?php
							...
							while($row = mysqli_fetch_array($res))
								{
									echo("<option> "."  ".$row['Email-id']."</option>");
								}
						?>
					</select><br><br>
